    ...
    def get_worked_days(self, form, emp_id, emp_salary, mes, ano):
        ...
        self.env.cr.execute(
            '''select sum(number_of_days) from hr_payslip_worked_days as p
                left join hr_payslip as r
                on r.id = p.payslip_id
                where r.employee_id = %s
                and (to_char(date_to,'mm')= %s)
    ...
    def get_employe_basic_info(self, emp_salary, cod_id, mes, ano):
        ...
        self.env.cr.execute(
            '''select sum(pl.total) from hr_payslip_line as pl
                ...
                where p.state = 'done' and (pl.code like %s)
    ...
    def get_salary(self, emp_id, emp_salary, cod_id, mes, ano):
        ...
        self.env.cr.execute(
            '''select sum(pl.total) from hr_payslip_line as pl
                ...
                left join resource_resource as r
                on r.id = emp.resource_id
                where p.state = 'done' and p.employee_id = %s
                and (pl.code like %s)
                and (to_char(p.date_to,'mm')=%s)
                and (to_char(p.date_to,'yyyy')=%s)
                group by r.name, p.date_to,emp.id
                ''', (emp_id, cod_id, mes, ano,))
    ...
    def get_employee2(self, form):
        ...
        if id_data is None:
            ...
            for index in id_data:
                #1 RUT 2 PRIMER 3 SEGUNDO 4 APELLIDO 5 SEGUNDO APELLIDO 
