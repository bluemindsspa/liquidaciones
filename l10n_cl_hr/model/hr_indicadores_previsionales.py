...
class hr_indicadores_previsionales(models.Model):
    ...
    update_date = fields.Date(string="Fecha actualizada", readonly=True)
    ...
    def update_document(self):
        ...
        try:
            ...
            # uf_one = clear_string(letters[0].select("strong")[2].get_text())
            ...
            # self.tope_imponible_ips = string_divide(clear_string(letters[2].select("strong")[2].get_text()), uf_one, 2)
            ...
            # self.asignacion_familiar_monto_a = clear_string(letters[9].select("td")[5].get_text())
            # self.asignacion_familiar_monto_b = clear_string(letters[9].select("td")[8].get_text())
            # self.asignacion_familiar_monto_c = clear_string(letters[9].select("td")[11].get_text())

            # self.asignacion_familiar_primer = clear_string(letters[9].select("td")[6].get_text())[1:]
            # self.asignacion_familiar_segundo = clear_string(letters[9].select("td")[9].get_text())[6:]
            # self.asignacion_familiar_tercer = clear_string(letters[9].select("td")[12].get_text())[6:]

            # # # 8 TASA COTIZACIÓN OBLIGATORIO AFP
            # self.tasa_afp_capital = clear_string(letters[8].select("strong")[8].get_text())
            # self.tasa_sis_capital = clear_string(letters[8].select("strong")[9].get_text())

            # self.tasa_afp_cuprum = clear_string(letters[8].select("strong")[11].get_text().replace(" ", '').replace("%", '').replace("1ff8", ''))
            # self.tasa_sis_cuprum = clear_string(letters[8].select("strong")[12].get_text())

            # self.tasa_afp_habitat = clear_string(letters[8].select("strong")[14].get_text())
            # self.tasa_sis_habitat = clear_string(letters[8].select("strong")[15].get_text())

            # self.tasa_afp_planvital = clear_string(letters[8].select("strong")[17].get_text())
            # self.tasa_sis_planvital = clear_string(letters[8].select("strong")[18].get_text())

            # self.tasa_afp_provida = clear_string(letters[8].select("strong")[20].get_text().replace(" ", '').replace("%", '').replace("1ff8", ''))
            # self.tasa_sis_provida = clear_string(letters[8].select("strong")[21].get_text())

            # self.tasa_afp_modelo = clear_string(letters[8].select("strong")[23].get_text())
            # self.tasa_sis_modelo = clear_string(letters[8].select("strong")[24].get_text())
            ...
            # self.tasa_independiente_capital = clear_string(letters[8].select("strong")[10].get_text())[:5]
            # self.tasa_independiente_cuprum = clear_string(letters[8].select("strong")[13].get_text())
            # self.tasa_independiente_habitat = clear_string(letters[8].select("strong")[16].get_text())
            # self.tasa_independiente_planvital = clear_string(letters[8].select("strong")[19].get_text())
            # self.tasa_independiente_provida = clear_string(letters[8].select("strong")[22].get_text())
            # self.tasa_independiente_modelo = clear_string(letters[8].select("strong")[25].get_text())
            # self.tasa_independiente_uno = clear_string(letters[8].select("strong")[28].get_text())
            
            # descoment in error
            date = fields.Date.context_today(self)
            date_con = date - relativedelta(months=1)
            date_month = self.month
            date_year = self.year
            if not date_month in ['10','11','12']:
                response = requests.get(f"https://api.gael.cloud/general/public/previred/0{date_month}{date_year}")
            else:
                response = requests.get(f"https://api.gael.cloud/general/public/previred/{date_month}{date_year}")
            data = response.json()
            # # UF
            self.uf = clear_string(data['UFValPeriodo'])
            # # UTM
            self.utm = clear_string(data['UTMVal'])
            # # UTA
            self.uta = clear_string(data['UTAVal'])
            
            # # RENTAS TOPES IMPONIBLES UF
            self.tope_imponible_afp = clear_string(data['RTIAfpUF'])
            self.tope_imponible_ips = clear_string(data['RTIIpsUF'])
            self.tope_imponible_seguro_cesantia = clear_string(data['RTISegCesUF'])
            
            # # # 4 RENTAS TOPES IMPONIBLES
            self.sueldo_minimo = clear_string(data['RMITrabDepeInd'])
            self.sueldo_minimo_otro = clear_string(data['RMIMen18May65'])
            
            self.contrato_plazo_indefinido_empleador = clear_string(data['AFCCpiEmpleador'])
            self.contrato_plazo_indefinido_trabajador = clear_string(data['AFCCpiTrabajador'])
            self.contrato_plazo_fijo_empleador = clear_string(data['AFCCpfEmpleador'])
            self.contrato_plazo_indefinido_empleador_otro = clear_string(data['AFCCpi11Empleador'])

            # # # Ahorro Previsional Voluntario
            self.tope_mensual_apv = clear_string(data['APVTopeMensUF'])
            self.tope_anual_apv = clear_string(data['APVTopeAnuUF'])

            # # # 5 DEPÓSITO CONVENIDO
            self.deposito_convenido = clear_string(data['DepConvenidoUF'])
            
            # # ASIGNACIÓN FAMILIAR
            self.asignacion_familiar_monto_a = clear_string(data['AFamTramoAMonto'])
            self.asignacion_familiar_monto_b = clear_string(data['AFamTramoBMonto'])
            self.asignacion_familiar_monto_c = clear_string(data['AFamTramoCMonto'])
            
            self.asignacion_familiar_primer = clear_string(data['AFamTramoAHasta'])
            self.asignacion_familiar_segundo = clear_string(data['AFamTramoBHasta'])
            self.asignacion_familiar_tercer = clear_string(data['AFamTramoCHasta'])
            
            # # TASA COTIZACIÓN OBLIGATORIO AFP
            self.tasa_afp_capital = clear_string(data['AFPCapitalTasaDep'])
            self.tasa_sis_capital = clear_string(data['AFPCapitalTasaSIS'])
            
            self.tasa_afp_cuprum = clear_string(data['AFPCuprumTasaDep'])
            self.tasa_sis_cuprum = clear_string(data['AFPCuprumTasaSIS'])

            self.tasa_afp_habitat = clear_string(data['AFPHabitatTasaDep'])
            self.tasa_sis_habitat = clear_string(data['AFPHabitatTasaSIS'])

            self.tasa_afp_planvital = clear_string(data['AFPPlanVitalTasaDep'])
            self.tasa_sis_planvital = clear_string(data['AFPPlanVitalTasaSIS'])

            self.tasa_afp_provida = clear_string(data['AFPProVidaTasaDep'])
            self.tasa_sis_provida = clear_string(data['AFPProVidaTasaSIS'])

            self.tasa_afp_modelo = clear_string(data['AFPModeloTasaDep'])
            self.tasa_sis_modelo = clear_string(data['AFPModeloTasaSIS'])

            self.tasa_afp_uno = clear_string(data['AFPUnoTasaDep'])
            self.tasa_sis_uno = clear_string(data['AFPUnoTasaSIS'])

            self.tasa_independiente_capital = clear_string(data['AFPCapitalTasaInd'])
            self.tasa_independiente_cuprum = clear_string(data['AFPCuprumTasaInd'])
            self.tasa_independiente_habitat = clear_string(data['AFPHabitatTasaInd']) 
            self.tasa_independiente_planvital = clear_string(data['AFPPlanVitalTasaInd'])
            self.tasa_independiente_provida = clear_string(data['AFPProVidaTasaInd'])
            self.tasa_independiente_modelo = clear_string(data['AFPModeloTasaInd'])
            self.tasa_independiente_uno = clear_string(data['AFPUnoTasaInd'])
            
        except ValueError as err:
            _logger.exception(f'A ocurrido un error en la consulta de la API PREVIRED: \n {err}')
