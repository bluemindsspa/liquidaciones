...
class hr_contract(models.Model):
    ...
    type_id = fields.Many2one('hr.contract.type', string="Categoria de empleado", required=True, help="Employee category", default=lambda self: self.env['hr.contract.type'].search([], limit=1))
    causal_id = fields.Many2one('hr.causal.termino', string="Causal termino contrato", default=lambda self: self.env['hr.causal.termino'].search([], limit=1))
