...
class WizardExportCsvPrevired(models.TransientModel):
    ...
    def get_cost_center(self, contract):
        cost_center = "0"
        if hasattr(contract, 'analytic_account_id'):
    ...
    def action_generate_csv(self):
        ...
        for payslip in payslip_recs:
            ...
            line_employee = [self._acortar_str(rut, 11), 
                             ...
                             int(self.get_imponible_afp_2(payslip and payslip[0] or False, self.get_payslip_lines_value_2(payslip,'TOTIM'), self.get_payslip_lines_value_2(payslip,'IMPLIC'))) if payslip.contract_id.isapre_id.codigo=='07' else "0",
