...
class report_hr_salary_employee_bymonth(models.AbstractModel):
    _name = 'report.l10n_cl_hr_electronic_book.report_hrsalarybymonth'
    ...
    def get_salary(self, emp_id, emp_salary, cod_id, mes, ano):
        ...
        and (to_char(p.date_to,'mm')=%s) and (to_char(p.date_to,'yyyy')=%s) and p.company_id = %s
        group by r.name, p.date_to,emp.id''', (emp_id, cod_id, mes, ano, company))
    ...
    def get_employee2(self, form):
        ...
        cont = 0
        self.env.cr.execute(
            '''select emp.id, emp.identification_id, emp.firstname, emp.middle_name, emp.last_name, emp.mothers_name, r.analytic_account_id, r.id, p.id
            from hr_payslip as p left join hr_employee as emp on emp.id = p.employee_id
            left join hr_contract as r on r.id = p.contract_id
            where p.state = 'done'  and (to_char(p.date_to,'mm')=%s)
            and (to_char(p.date_to,'yyyy')=%s) and p.company_id = %s
            group by emp.id, emp.name, emp.middle_name, emp.last_name, emp.mothers_name, emp.identification_id, r.analytic_account_id, r.id, p.id
            order by r.analytic_account_id, last_name''', (last_month, last_year, company))
        ...
        if id_data is None:
            ...
            for index in id_data:
                ...
                emp_salary.append(id_data[cont][7]) # Contract id
                emp_salary.append(id_data[cont][8]) # Payslip id
